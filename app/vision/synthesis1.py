...
def get_autopipeline():
    ...
    pipeline = load_ip_adapter(pipeline=pipeline,adapter="h94/IP-Adapter",subfolder_model="sdxl_models", weights="ip-adapter_sdxl.bin" )
    pipeline = set_ip_scale(pipeline, ip_scale=1.0 )
...
global_pipeline = get_autopipeline()
